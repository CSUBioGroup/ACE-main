...
data_dir2= '/home/yxh/gitrepo/multi-omics-matching/Senst_exp_inputs/dogma_bridge_num/'
out_dir = '/home/yxh/gitrepo/multi-omics-matching/neurips2021_multimodal_topmethods-main/output/pretrain/stabmap/dogma_bridge_num/'
...
for (del_size in c(0.1, 0.2, 0.4, 0.8)){
  for (r in c(0, 1, 2)){
      train_idx = read.table(paste0(data_dir2, glue('DelSize={del_size}_r={r}_ids.csv')), sep=',', header=T)$X0 + 1
    
      # split
      mult_rna_count = rna_count[, train_idx]
      mult_adt_count = adt_count[, train_idx]
      mult_atac_count = atac_count[, train_idx]
      single_rna_count = rna_count[, test_idx]
      single_adt_count = adt_count[, test_idx]
      single_atac_count = atac_count[, test_idx]
      ...
      colnames(single_rna_count) = paste0('gex_', cell_names$x[test_idx])
      colnames(single_adt_count) = paste0('adt_', cell_names$x[test_idx])
      colnames(single_atac_count) = paste0('atac_', cell_names$x[test_idx])
      ...
      single_rna_lognorm = as(data_rna[['RNA']]@data[, (n_mult+1):(n_mult+n_single)], 'dgCMatrix')
      single_adt_lognorm = as(data_adt[['RNA']]@data[, (n_mult+1):(n_mult+n_single)], 'dgCMatrix') 
      single_atac_lognorm = as(GetAssayData(data_atac)[variable_atac_features, (n_mult+1):(n_mult+n_single)], 'dgCMatrix') 
