...
# running command
# python sec1_case3.py --DATASETS_PATH "/home/yanxh/data/Seurat_demo_data/bm_cite/" \
#                 --PRETRAIN_PATH "/home/yanxh/gitrepo/multi-omics-matching/neurips2021_multimodal_topmethods-main/output/pretrain/matchclot/case3_bm-cite" \
...
if __name__ == "__main__":
    ...
    for p in [0.1, 0.2, 0.4, 0.8]:
        for repeat in range(3):
            new_train_idx = np.load(f'/home/yanxh/gitrepo/multi-omics-matching/Senst_exp_inputs/case3/bm-cite/p={p}_r={repeat}_new_train_idx.npy')
            test_rna_idx = np.load(f'/home/yanxh/gitrepo/multi-omics-matching/Senst_exp_inputs/case3/bm-cite/p={p}_r={repeat}_test_rna_idx.npy')
            test_adt_idx = np.load(f'/home/yanxh/gitrepo/multi-omics-matching/Senst_exp_inputs/case3/bm-cite/p={p}_r={repeat}_test_adt_idx.npy')
            
            input_train_mod1 = sc.AnnData(rna_count_mat[new_train_idx], obs=meta_data.iloc[new_train_idx])
            input_train_mod2 = sc.AnnData(adt_count_mat[new_train_idx], obs=meta_data.iloc[new_train_idx])
            input_test_mod1  = sc.AnnData(rna_count_mat[test_rna_idx], obs=meta_data.iloc[test_rna_idx])
            input_test_mod2  = sc.AnnData(adt_count_mat[test_adt_idx], obs=meta_data.iloc[test_adt_idx])
            input_train_mod1.obs_names = input_train_mod2.obs_names = cell_names[new_train_idx]
            input_test_mod1.obs_names = cell_names[test_rna_idx]
            input_test_mod2.obs_names = cell_names[test_adt_idx]
            ...
            idx1, idx2 = train_test_split(np.arange(input_train_mod2.shape[0]), test_size=0.2, stratify=input_train_mod2.obs['celltype.l2'])
            input_train_mod2.obs[split_trainval_by] = np.array(['train']*input_train_mod2.shape[0])
            input_train_mod2.obs[split_trainval_by][idx2] = 'val'
