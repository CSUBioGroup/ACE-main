...
par['output_pretrain'] = os.path.join(
    root_dir, 
    'output/pretrain/clue/case4_dogma/')
...
for p in [0.1, 0.2, 0.4, 0.8]:  # training propor: [0] + [0.9, 0.8, 0.6, 0.2]
    for repeat in range(3):
        ...
        save_dir = os.path.join(par['output_pretrain'], f'p={p}-repeat={repeat}')
        ...
        ad_rna_test = sc.AnnData(sps.csr_matrix(rna_count_mat[test_rna_idx]), obs=meta_data.iloc[test_rna_idx])
        ad_adt_test = sc.AnnData(sps.csr_matrix(adt_count_mat[test_adt_idx]), obs=meta_data.iloc[test_adt_idx])
        ad_atac_test = sc.AnnData(sps.csr_matrix(atac_count_mat[test_atac_idx]), obs=meta_data.iloc[test_atac_idx])
