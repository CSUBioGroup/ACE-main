...
input_dir = '../../data/Other_inputs/trimodal_case4'
log_dir = '../../outputs/trimodal_case4'

for p in [0.1, 0.2, 0.4, 0.8]:
    for repeat in range(3):
        new_train_idx = np.load(f'{input_dir}/p={p}_r={repeat}_new_train_idx.npy')
        test_rna_idx  = np.load(f'{input_dir}/p={p}_r={repeat}_test_rna_idx.npy')
        test_adt_idx  = np.load(f'{input_dir}/p={p}_r={repeat}_test_adt_idx.npy')
        test_atac_idx = np.load(f'{input_dir}/p={p}_r={repeat}_test_atac_idx.npy')

        save_dir = join(log_dir, f'p={p}_r={repeat}')
        if os.path.exists(join(save_dir, 'stage1')):
            continue
        os.makedirs(save_dir, exist_ok=True)

        ad_mult_rna = sc.AnnData(rna_count_mat[new_train_idx], obs=meta_data.iloc[new_train_idx])
        ad_rna_test = sc.AnnData(rna_count_mat[test_rna_idx], obs=meta_data.iloc[test_rna_idx])
        ad_mult_adt = sc.AnnData(adt_count_mat[new_train_idx], obs=meta_data.iloc[new_train_idx])
        ad_adt_test = sc.AnnData(adt_count_mat[test_adt_idx], obs=meta_data.iloc[test_adt_idx])
        ad_mult_atac = sc.AnnData(atac_count_mat[new_train_idx], obs=meta_data.iloc[new_train_idx])
        ad_atac_test = sc.AnnData(atac_count_mat[test_atac_idx], obs=meta_data.iloc[test_atac_idx])
